...
def load_resources():
     try:
         # PENTING: Pastikan file index dan label ada di folder Streamlit Anda
         ...
         # Jika Anda melihat pesan ini di Colab, artinya file .pkl atau .json hilang
         # st.error(f"File model ({INDEX_FILE} atau {LABEL_FILE}) tidak ditemukan. Pastikan sudah diunggah.")
         return None, None, None, None, None # Mengembalikan None untuk menghindari crash
 ...
 def pil_to_cv2_gray(pil_img):
     ...
     rgb_img = np.array(pil_img.convert('RGB'))[:, :, ::-1] # PIL to BGR
     gray = cv2.cvtColor(rgb_img, cv2.COLOR_BGR2GRAY)
     return gray.astype(np.uint8)
 
 def deskew(image):
     # Fungsi Deskew (disalin dari Cell 3)
 ...
 def preprocess_image(pil_img):
     # Melakukan Preprocessing lengkap (Blur -> Threshold -> Deskew -> Resize)
 ...
 def extract_orb(image):
     # Ekstraksi ORB dengan Canny Boosted (dari Cell 4)
 ...
 def predict_ratio(des_query, index):
     # Fungsi Prediksi menggunakan Rasio Lowe (dari Cell 4)
     scores = {}
         ...
         try:
             ...
             for pair in matches:
                 ...
                 if m.distance < RATIO_THRESH * n.distance:
                     good_matches += 1
             scores[label] = scores.get(label, 0) + good_matches
         except:
             continue
     if not scores: return None
     predicted_label_id = max(scores.items(), key=lambda x: x[1])[0]
     return ID_TO_LABEL[predicted_label_id]
...
if uploaded_file is not None:
     ...
     if ORB_INDEX is None:
         st.error("🚨 Model tidak berhasil dimuat! Harap unggah orb_index.pkl dan label_map.json ke direktori ini.")
         st.stop()
     ...
     try:
         ...
         with col1:
             ...
             if des_query is not None and len(des_query) > 0:
                 prediction_label = predict_ratio(des_query, ORB_INDEX)
